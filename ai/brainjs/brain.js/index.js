...
class Dropout extends Filter {
  ...
  predict() {
    ...
    const { result, dropouts } = this.predictKernelMap(this.inputLayer.weights);
    this.weights = result;
  ...
  const cell = tanh$1(
    add$1(
      add$1(
        ...
        multiply$1(cellPeepholes, multiplyElement$1(resetGate, recurrentInput))
      ),
      cellBias
    )
  );
...
function rnnCell(settings, input, recurrentInput) {
  ...
  return relu$1(
    add$1(
      add$1(multiply$1(weight, input), multiply$1(transition, recurrentInput)),
      bias
    )
  );
}
...
class NeuralNetwork {
  ...
  async trainAsync(data, options = {}) {
    ...
    return await new Promise((resolve, reject) => {
      try {
        const thawedTrain = new dist.Thaw(
...
class RNN {
  ...
  train(data, trainOpts = {}) {
    ...
    let inputs;
    if (
      (_a = this.options) === null || _a === void 0 ? void 0 : _a.dataFormatter
    ) {
